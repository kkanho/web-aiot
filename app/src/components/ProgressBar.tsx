...
const ProgressBar = ({value, offset, min, max, ...props}: ProgressBarProps) => {
    ...
    const percentage = value / (max + offset*2) * 100
    const progressBarStyle = {
        width: `${percentage > max || percentage > 100 ? 100 : percentage < 10 ? 10 : percentage}%`,
    ...
    return (
        <div className="w-full h-2.5 bg-gray-600 rounded-full" {...props}>
            <div
                className={`h-2.5 rounded-full ease-in-out ${value > min + offset && value < max - offset ? 'bg-green-400' :
                    value < min - offset || value > max + offset ? 'bg-red-400' :
                    'bg-yellow-400'}`}
